...
# cod folosit pentru a genera permutările parolei
def genPermutation(i, s, used, curr, st):
    if i == len(s):
        # adaug permutarea la lista de permutari
        st.add("".join(curr))
        return

    for j in range(len(s)):
        if not used[j]:
            # marchez caracterul drept folosit
            used[j] = True
            curr.append(s[j])

            # merg la urmatorul caracter
            genPermutation(i + 1, s, used, curr, st)

            # backtrack la caracterul anterior
            used[j] = False
            curr.pop()
def findPermutation(s):
    # vedem daca caracterul e folosit
    used = [False] * len(s)
    st = set()
    curr = []

    # începem procesul de recursivitate
    genPermutation(0, s, used, curr, st)

    # transformam setul într-o listă
    return list(st)
